...
def add_to_wish():
    ...
    if (r != []):
        ...
        customer.execute(
            '''Insert Into Wishlist(Product_ID,Product_Name,Description,Quantity,Price, Customer_id) values(?, ?, ?, ?, ?, ?)''',
...
def add_to_cart():
    ...
    if (r != []):
        ...
        customer.execute(f'''select Quantity  from Products where Product_ID =?''', (product_id,))
        ...
        if quan[0]==0:
            ...
            _status = "valid"
            customer.execute(
                f'''Insert Into Cart(Product_ID,Product_Name,Description,Quantity,Price, Customer_id) values(?, ?, ?, ?, ?, ?)''', (product_id, product_name, product_desc, product_quant, product_price,customer_id))
            customer.execute("Update Products set Quantity=Quantity-1 where Product_Id=?", (product_id,))
            con.commit()
...
def check_cart(prid):
    ...
    #     product_id = request.json.get("pid")
    p_id = prid
    customer.execute('''select * from Cart where Product_ID =?''', (p_id,))
    r = customer.fetchall()
    if r != []:
...
def cart():
    ...
    customer.execute(f'''select * from Cart where Customer_id =?''', (value,))
    r = customer.fetchall()
    print(r)
    print(value)
...
def functions_of_Cart():
    ...
    customer.execute('''Select Quantity from Cart  where Product_Id=? and Customer_id =?''',(prodid,value))
    r=customer.fetchone()
    quan=r[0]
    print(quan)
    if funtiopass=='rem':
        if quan>1:
            customer.execute("Update Cart set Quantity=Quantity-1 where Product_Id=? and Customer_id =?",(prodid,value))
            customer.execute("Update Products set Quantity=Quantity+1 where Product_Id=?", (prodid,))
            con.commit()
        elif quan==1:
            customer.execute("Delete from Cart where Product_Id=? and Customer_id =?",(prodid,value))
            con.commit()

    elif funtiopass=='add':
        customer.execute('''Select Quantity from Products  where Product_Id=?''', (prodid,))
        r = customer.fetchone()
        pquan = r[0]
        print(pquan)
        if pquan>0:
        ...
        customer.execute("select Quantity from Cart where Customer_id=? and Product_Id=?",(value,prodid))
        qaunt=customer.fetchone()
        customer.execute("Delete from Cart where Product_Id=? and Customer_id =?",(prodid,value))
        customer.execute("Update Products set Quantity=Quantity+? where Product_Id=?", (qaunt[0],prodid,))
        con.commit()
...
def display_product():
    ...
    customer.execute('Select * from Products where Product_ID =?', (pid,))
    a = customer.fetchone()
    print(a)
    if a == None:
...
def invoice():
    ...
    if(paymethod=="Cash on Delivery"):
        ...
        customer.execute("insert into Orders values (?, ?, ?, ?, ?)", (tr, date, x, custid, paymethod))
        customer.execute('delete from Cart where Customer_Id=?', (custid,))
        con.commit()
        return render_template('ogc_home.html')
    if(paymethod):
        value = session['custid']
        ...
        tr=int(random.randint(10000, 99999))
        date = str(datetime.now().date())
        customer.execute("insert into Orders values (?, ?, ?, ?, ?)", (tr, date, x, value, paymethod))
        customer.execute('delete from Cart where Customer_Id=?', (value,))
        con.commit()
...
def wishlist():
    value = session['custid']
    customer.execute(f'''select * from Wishlist where Customer_id =?''', (value,))
    r = customer.fetchall()
...
def functions_of_wish():
    ...
    if funtiopass == 'totrem':
        ...
        customer.execute("Delete from Wishlist where Product_Id=? and Customer_id=?", (prodid,value))
        con.commit()
        return jsonify({"Status": 200, "message": "Removed Succesfully"})
    elif funtiopass=='add':
        customer.execute('''Select * from Cart  where Product_Id=? and Customer_id=?''', (prodid,value))
        val = customer.fetchall()
        print(val)
        if val==():
            customer.execute("select * from Products where Product_ID=?",(prodid,))
            r=customer.fetchone()
            customer.execute("Insert into Cart values(?,?,?,?,?,?)",(r[0],r[1],r[2],1,r[4],value))
            con.commit()
            print("Im worked",r)
...
def profile_update():
    value = session['custid']
    customer.execute(f'''select * from Registration where Customer_id =?''', (value,))
    r = customer.fetchone()
...
def transaction():
    value=session['custid']
    condition=f"Customer_id='{value}'"
    customer.execute("SELECT SUM(Price * Quantity) FROM Cart WHERE Customer_id = ?",(value,))
    r=customer.fetchall()
    session['total']=r[0]
    return render_template('ogc_transaction.html',r=r[0])
...
@login_required
def order():
    customer.execute(f'''select * from Orders where Customer_ID =?''',(session['custid'],))
    r = customer.fetchall()
    print(r)
    if r!=[]:
...
def removeCustomer():
    ...
    customer.execute("Delete from Registration Where Customer_Id=?",(cid,))
    con.commit()
...
def display_customer():
    ...
    customer.execute('Select * from Registration where Customer_id =?',(cid,))
    a=customer.fetchone()
    #print(a)
    if a==None:
...
def display_product_id():
    cid = request.json.get("id")
    customer.execute('Select * from Products where Product_ID =?', (cid,))
    a = customer.fetchone()
    print(a)
    if a == None:
...
def display_customer_name():
    cid=request.json.get("id")
    customer.execute(f'''select * from Registration where Customer_Name Like ?''',('%' + cid + '%',))
    a=customer.fetchall()
    if a == []:
        return jsonify({"message": "No Customer Found", "Status": 201})
    elif a != []:
...
def display_product_name():
    cid = request.json.get("id")
    customer.execute(f'''select * from Products where Product_name Like ?''', ('%' + cid + '%',))
    a = customer.fetchall()
    print(a)
    if a == []:
        return jsonify({"message": "No Product Found", "Status": 201})
...
def display_customer_emails():
    ...
    customer.execute(f'''select * from Registration where Email like ? ''',('%'+domain+'%',))
    a=customer.fetchall()
    print(domain)
    if a==[]:
        return jsonify({"message":"No Customer Found","Status":201})
...
def display_prod_category():
    domain = request.json.get('id')
    customer.execute(f'''select * from Products where Description like ? ''', ('%' + domain + '%',))
    a = customer.fetchall()
    print(domain)
    if a == []:
        return jsonify({"message": "No Product Found", "Status": 201})
...
def butoncheck():
    ...
    customer.execute('select Status from Registration where Customer_id=?',(custid,)) 
    b=customer.fetchone()
    
    if b[0]=="Active":
        customer.execute('UPDATE Registration SET Status="Inactive" WHERE Customer_id=? ', (custid,))
        con.commit()
        print("hey")
        return jsonify({"status":"Inactive"})
    
    if b[0]=="Inactive":
        customer.execute('UPDATE Registration SET Status="Active" WHERE Customer_id=? ', (custid,))
        con.commit()
...
def update_product():
    ...
    if request.method == "POST":
        ...
        customer.execute('''Update Products SET Product_Name =?,Description =?,Quantity =?,Price =? Where Product_ID =?''', ( product_name, product_desc, product_quant, product_price,product_id))
        con.commit()
...
def delete_product():
    ...
    if r == []:
        ...
        customer.execute('''Delete from Products where Product_ID =?''', (product_id,))
        con.commit()
...
def admin_update_product():
    ...
    customer.execute(f'''select * from Products where Product_ID =?''', (value,))
    r = customer.fetchone()
    print(r)
...
def searching():
    ...
    print(value)
    customer.execute('Select * from Products where Product_Name like ? ',( value + '%',))
    r=customer.fetchall()
    print(r)
    keys_list = ['id', 'name', 'desc', 'quant', 'price']
    temp_dict = {}
...
def customerinactive():
    value=session['custid']
    print(value)
    customer.execute(f'''update Registration set Status='Inactive' where Customer_id=? ''',(value,))
    con.commit()
    session.pop("custid",None)
    return render_template('ogc_login.html')
...
def admvalidate():
    ...
    customer.execute('select Admin_Id,Password from Admin where Admin_Id=? and Password=?',(adminid,password,))
...
def adminprof():
    ...
    print(id)
    customer.execute('select * from Admin where Admin_Id=?',(id,))
    adminget=customer.fetchone()
    print(adminget)
...
def activate():
    value=session['custid']
    customer.execute(f'''update Registration set Status='Active' where Customer_id=? ''',(value,))
    con.commit()
    return render_template('ogc_login.html')
...
def adminnewregister(name, password, contact_no, c_id):
    customer.execute('''Insert Into Admin values(?, ?, ?, ?)''', (c_id,name,password,contact_no))
    con.commit()
